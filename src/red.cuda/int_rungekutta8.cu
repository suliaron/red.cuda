...
void rungekutta8::call_kernel_calc_ytemp_for_fr(int n_var, int r)
{
	int idx = 0;
	for (int i = 0; i < 2; i++)
	{
		...
		case 1:
			idx = 1;		
			rk8_kernel::calc_ytemp_for_f1<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, a[idx]);
			break;
		case 2:
			idx = 2;
			rk8_kernel::calc_ytemp_for_f2<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f1, a[idx], a[idx+1]);
			break;
		case 3:
			idx = 4;
			rk8_kernel::calc_ytemp_for_f3<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f2, a[idx], a[idx+2]);
			break;
		case 4:
			idx = 7;
			rk8_kernel::calc_ytemp_for_f4<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f2, f3, a[idx], a[idx+2], a[idx+3]);
			break;
		case 5:
			idx = 11;
			rk8_kernel::calc_ytemp_for_f5<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f3, f4, a[idx], a[idx+3], a[idx+4]);
			break;
		case 6:
			idx = 16;
			rk8_kernel::calc_ytemp_for_f6<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f3, f4, f5, a[idx], a[idx+3], a[idx+4], a[idx+5]);
			break;
		case 7:
			idx = 22;
			rk8_kernel::calc_ytemp_for_f7<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f4, f5, f6, a[idx], a[idx+4], a[idx+5], a[idx+6]);
			break;
		case 8:
			idx = 29;
			rk8_kernel::calc_ytemp_for_f8<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f3, f4, f5, f6, f7, a[idx], a[idx+3], a[idx+4], a[idx+5], a[idx+6], a[idx+7]);
			break;
		case 9:
			idx = 37;
			rk8_kernel::calc_ytemp_for_f9<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f3, f4, f5, f6, f7, f8, a[idx], a[idx+3], a[idx+4], a[idx+5], a[idx+6], a[idx+7], a[idx+8]);
			break;
		case 10:
			idx = 46;
			rk8_kernel::calc_ytemp_for_f10<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f3, f4, f5, f6, f7, f8, f9, a[idx], a[idx+3], a[idx+4], a[idx+5], a[idx+6], a[idx+7], a[idx+8], a[idx+9]);
			break;
		case 11:
			idx = 56;
			rk8_kernel::calc_ytemp_for_f11<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f5, f6, f7, f8, f9, a[idx], a[idx+5], a[idx+6], a[idx+7], a[idx+8], a[idx+9]);
			break;
		case 12:
			idx = 67;
			rk8_kernel::calc_ytemp_for_f12<<<grid, block>>>(n_var, ytemp, dt_try, y_n, f0, f3, f4, f5, f6, f7, f8, f9, f11, a[idx], a[idx+3], a[idx+4], a[idx+5], a[idx+6], a[idx+7], a[idx+8], a[idx+9], a[idx+11]);
			break;
		default:
...
void rungekutta8::call_kernel_calc_error(int n_var)
{
	for (int i = 0; i < 2; i++)
	{
		var_t *err = d_err[i];
		...
		rk8_kernel::calc_error<<<grid, block>>>(n_var, err, f0, f10, f11, f12);
		cudaError cudaStatus = HANDLE_ERROR(cudaGetLastError());
		if (cudaSuccess != cudaStatus)
		{
...
void rungekutta8::call_kernel_calc_y_np1(int n_var)
{
	for (int i = 0; i < 2; i++)
	{
		var_t* y_n   = (var_t*)ppd->sim_data->d_y[i];
...
//DEBUG CODE
			//printf("r: %2d d_ytemp[0]: \n", r);
			//kernel_print_array<<<1, 1>>>(NDIM * n_total, (var_t *)d_ytemp[0]);
			//printf("r: %2d d_ytemp[1]: \n", r);
			//kernel_print_array<<<1, 1>>>(NDIM * n_total, (var_t *)d_ytemp[1]);
...
//DEBUG CODE
		//printf("r: %2d d_yout[0]\n", r);
		//kernel_print_array<<<1, 1>>>(NDIM * n_total, (var_t*)ppd->sim_data->d_yout[0]);
		//printf("r: %2d d_yout[1]\n", r);
		//kernel_print_array<<<1, 1>>>(NDIM * n_total, (var_t*)ppd->sim_data->d_yout[1]);
		//cudaDeviceSynchronize();
//DEBUG CODE END
...
//DEBUG CODE
			//printf("r: %2d d_err[0]: \n", r);
			//kernel_print_array<<<1, 1>>>(NDIM * n_total, d_err[0]);
			//printf("r: %2d d_err[1]: \n", r);
			//kernel_print_array<<<1, 1>>>(NDIM * n_total, d_err[1]);
			//cudaDeviceSynchronize();
//DEBUG CODE END
...
void c_rungekutta8::call_kernel_calc_ytemp(int n_var, int r)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		c_rk8_kernel::calc_ytemp<<<grid, block>>>(n_var, r, idx_array[r], i*r_max, dt_try, y_n, dydt, ytemp);
		cudaError cudaStatus = HANDLE_ERROR(cudaGetLastError());
		if (cudaSuccess != cudaStatus)
		{
...
void c_rungekutta8::call_kernel_calc_error(int n_var)
{
	for (int i = 0; i < 2; i++)
	{
		...
		c_rk8_kernel::calc_error<<<grid, block>>>(n_var, f0, f10, f11, f12, d_err[i]);
		cudaError cudaStatus = HANDLE_ERROR(cudaGetLastError());
		if (cudaSuccess != cudaStatus)
		{
