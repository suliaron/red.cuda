...
static __global__
	void calc_yHat(int n, const var_t *y_n, const var_t *f1, const var_t *f2, const var_t *f3, const var_t *f4, var_t b0, var_t b1, var_t *y_hat)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = gridDim.x * blockDim.x;
...
static __global__
	void calc_error(int n, const var_t *f4, const var_t* f5, var_t *result)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = gridDim.x * blockDim.x;
...
void rungekutta4::call_kernel_calc_ytemp_for_fr(int n_var, int r)
{
	for (int i = 0; i < 2; i++)
	{
		...
		rk4_kernel::sum_vector<<<grid, block>>>(n_var, y_n, fr, a[r] * dt_try, ytemp);
...
void rungekutta4::call_kernel_calc_yHat(int n_var)
{
	for (int i = 0; i < 2; i++)
	{
		var_t *y_n	 = (var_t*)ppd->sim_data->d_y[i];
		...
		rk4_kernel::calc_yHat<<<grid, block>>>(n_var, y_n, f1, f2, f3, f4, b[0] * dt_try, b[1] * dt_try, y_Hat);
		cudaError cudaStatus = HANDLE_ERROR(cudaGetLastError());
		if (cudaSuccess != cudaStatus)
		{
			throw string("rk4_kernel::calc_yHat failed");
		}
	}
}

void rungekutta4::call_kernel_calc_error(int n_var)
{
	for (int i = 0; i < 2; i++)
	{
		var_t *f4  = (var_t*)d_f[i][3];
		...
		rk4_kernel::calc_error<<<grid, block>>>(n_var, f4, f5, d_err[i]);
		cudaError cudaStatus = HANDLE_ERROR(cudaGetLastError());
		if (cudaSuccess != cudaStatus)
		{
...
ttt_t rungekutta4::step()
{
	...
	do
	{
		...
		call_kernel_calc_yHat(n_var_total);
