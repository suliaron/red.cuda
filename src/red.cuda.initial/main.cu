...
void populate_solar_system(body_disk_t& disk, pp_disk_t::sim_data_t *sd)
{
	...
	pp_disk_t::body_metadata_t body_md = {0, 0, 0, MIGRATION_TYPE_NO};
...
void populate_disk(ttt_t t0, body_disk_t& disk, pp_disk_t::sim_data_t *sd)
{
    ...
    uint32_t bodyIdx = 0;
	uint32_t bodyId  = 1;
	for (unsigned int body_type = BODY_TYPE_STAR; body_type < BODY_TYPE_N; body_type++)
	{
		for (unsigned int i = 0; i < disk.nBody[body_type]; i++, bodyIdx++, bodyId++)
		{
			body_md.id          = bodyId;
...
uint32_t Dvorak(nebula& n, body_disk_t& disk)
{
	...
	disk.nBody[BODY_TYPE_STAR        ] = 1;
	disk.nBody[BODY_TYPE_PROTOPLANET ] = 300;
...
uint32_t Birgit_scenario(body_disk_t& disk)
{
	...
	disk.nBody[BODY_TYPE_STAR        ] = 1;
	disk.nBody[BODY_TYPE_GIANTPLANET ] = 2;
	disk.nBody[BODY_TYPE_TESTPARTICLE] = 100;
	...
	type = BODY_TYPE_TESTPARTICLE;
	{
		...
		disk.oe_d[type].item[ORBITAL_ELEMENT_ECC ] = new uniform_distribution(rand(), 0.99, 0.9999);
		disk.oe_d[type].item[ORBITAL_ELEMENT_INC ] = new uniform_distribution(rand(), 0.0, 0.5);
		disk.oe_d[type].item[ORBITAL_ELEMENT_PERI] = new uniform_distribution(rand(), 0.0, 2.0 * PI);
		disk.oe_d[type].item[ORBITAL_ELEMENT_NODE] = new uniform_distribution(rand(), 0.0, 2.0 * PI);
		disk.oe_d[type].item[ORBITAL_ELEMENT_MEAN] = new uniform_distribution(rand(), 0.0, 1.0 * PI);
...
uint32_t Elke_binary_test(body_disk_t& disk)
{
	...
	int type = BODY_TYPE_STAR;
	{
		...
		for (unsigned int i = 0; i < disk.nBody[type]; bodyIdx++, i++)
		{
			disk.mig_type[bodyIdx] = MIGRATION_TYPE_NO;
			disk.stop_at[bodyIdx] = 0.0;
		}
...
void Chambers2001(string& dir, string& filename)
{
	...
	// Outer disk: r = 0.7, ... 2.0: the surface density is prop to r ^-3/2
	{
		...
		// Computes the physical quantities with the new mass
		{
			...
			{
				for (unsigned int i = 0; i < disk.nBody[type]; i++, bodyIdx++)
				{
		...
		for (uint32_t i = 1; i < n_body; i++)
		{
			...
			{				
				...
				if (1 == gp_counter)
				{
					disk.names[i] = "Saturn";
					sim_data->h_p[i].mass          = 1.0 * constants::SaturnToSolar;
					sim_data->h_p[i].radius        = 58232.0 * constants::KilometerToAu;
					sim_data->h_p[i].density       = tools::calc_density(sim_data->h_p[i].mass, sim_data->h_p[i].radius);
					ttt_t epoch = extract_from_horizon_output(ephemeris_major_planets::date_20150511::saturn_oe, sim_data->h_oe[i]);
				}
				gp_counter++;
